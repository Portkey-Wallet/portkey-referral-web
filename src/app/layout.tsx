...
const GA_TRACKING_ID_PRODUCTION = process.env.NEXT_PUBLIC_GA_ID_PRODUCTION;
const GA_TRACKING_ID_DEVELOPMENT = process.env.NEXT_PUBLIC_GA_ID_DEVELOPMENT;
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const gaTrackingId = process.env.NODE_ENV === 'production' ? GA_TRACKING_ID_PRODUCTION : GA_TRACKING_ID_DEVELOPMENT;

  return (
    <html lang="en">
      <Script src={`https://www.googletagmanager.com/gtag/js?id=G-7BKVT9ZZ5X`} strategy="afterInteractive" />
      <Script id="google-analytics" strategy="afterInteractive">
        {`
          window.dataLayer = window.dataLayer || [];
          function gtag(){window.dataLayer.push(arguments);}
